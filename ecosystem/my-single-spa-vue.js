...
(function (global) {
  ...
  global.mySingleSpaVue = function (options) {
    if (typeof options !== 'object') {
      throw new this.Error('mySingleSpaVue options must be a object');
    }

    let opts = Object.assign({}, defaultOpts, options);
    if (!options.Vue) {
      throw new this.Error('mySingleSpaVue must be passed a Vue');
    }
    if (!options.appOptions) {
      throw new this.Error('mySingleSpaVue must be passed a appOptions');
    }
